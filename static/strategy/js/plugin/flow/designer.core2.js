...
var Designer = {
            ...
            b.unbind("click").bind("click", function (c) {
                ...
                for (const key in def_local_1_data.elements) {
                    if (def_local_1_data.elements.hasOwnProperty(key)) {
                        ...
                        if (key === a.from.id) {
                            if (element.name == 'control' || element.name == 'branch') {
                                Designer.op.editShapeText(a, d);
        ...
        removeShape: function () {
            ...
            if (selectedList.length > 0) {
                ...
                for (let i = 0; i < selectedList.length; i++) {
                    ...
                    if (e.name == 'linker' && e.from.id) {
                        if (selectedList.length > 1 && e.name == 'linker' && uiData.elements[e.from.id].name == 'branch') {
                            UI.showTip('分支节点请单独删除');
                            return null
                        }
                        if (e.name == 'linker' && uiData.elements[e.from.id].name == 'branch' && e.to.id) {
                            parent.postMessage(JSON.stringify({ code: 6, value: { branchId: uiData.elements[e.from.id].id, nextNodeId: e.to.id } }), "*")
                        }
                        if (e.name == 'linker' && uiData.elements[e.from.id].name == 'branch' && !e.to.id) {
                            parent.postMessage(JSON.stringify({ code: 7, value: { branchId: uiData.elements[e.from.id].id } }), "*")
                        }
                    }
                    for (const key in uiData.elements) {
                        if (uiData.elements.hasOwnProperty(key)) {
                            const element = uiData.elements[key];
                            if (selectedList.length > 1 && element.name === 'linker' && element.from.id && uiData.elements[element.from.id].name === 'branch' && element.to.id && element.to.id === e.id) {
                                UI.showTip('分支节点的下游节点，请单独删除');
                                return null
                            }
                            if (element.name === 'linker' && element.from.id && uiData.elements[element.from.id].name === 'branch' && !element.to.id) {
                                UI.showTip('分支节点的下游节点，请先设置');
                                return null
                            }
                        }
                    }
                ...
                d = d.concat(f);
                localStorage.def_local_1_bk = sessionStorage.def_local_1;
                Model.remove(d)
    ...
    painter: {
        ...
        renderShapePath: function (a, b, c, d) {
            var e;
            console.log("b.drawIcon",a, b, c, d);
            if (c && b.drawIcon) {
