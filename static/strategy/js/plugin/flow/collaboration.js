...
var CLB = {
    ...
    doSend: function (c) {
        ...
        sessionStorage["def_local_" + chartId] = JSON.stringify(Model.define);
        sessionStorage["title_local_" + chartId] = d;
        sessionStorage["version_local_" + chartId] = chartVersion + 1;
        this.localSaveCount++;
        $("#saving_tip").html("<span style='color:rgb(245,174,67)'>已保存到本地</span>");
        sessionStorage["def_" + chartId] = JSON.stringify(Model.define);
    ...
    localToServer: function (c) {
        var a = sessionStorage["def_local_" + chartId];
        var b = sessionStorage["title_local_" + chartId];
        if (!a) {
            return
        } this.sending = true; 
        // $.ajax({
    ...
    findLocal: function () {
        ...
        var a = false;
        if (sessionStorage && sessionStorage["def_local_" + chartId]) {
            var d = sessionStorage["def_local_" + chartId];
            console.log("d = ", d)
            var c = parseInt(sessionStorage["version_local_" + chartId]);
            var b = parseInt(sessionStorage["title_local_" + chartId]);
            if (c >= chartVersion) {
                console.log("解析本地数据");
                Designer.open(d);
                this.localToServer();
    ...
    removeLocal: function () {
        sessionStorage.removeItem("def_local_" + chartId);
        sessionStorage.removeItem("title_local_" + chartId);
        sessionStorage.removeItem("version_local_" + chartId)
    },
