...
var Designer = {
        ...
        initShapes: function () {
            ...
            for (var f = 0; f < Schema.categories.length; f++) {
                ...
                if (g.name == 'dataSource') {
                    $("#shape_panel").append("<div class='panel_container' style='display: none;'><h3 class='panel_title'><div class='ico ico_accordion'></div>" + g.text + "</h3><div id='panel_" + g.name + "' class='content'></div></div>")
            ...
            function e(l, o) {
                ...
                var n = "<div class='panel_box' shapeName='" + l.name + "'><canvas class='panel_item' width='" + (Designer.config.panelItemWidth) + "' height='" + (Designer.config.panelItemHeight) + "'></canvas><p style='line-height: 18px;height: 18px; width: fit-content; float: left; margin-left: 18px;'>" + icon + '</p><p style=" width: fit-content; float: left;margin-left: 10px;">' + l.title + "</p></div>";
                    ...
                    $(document).bind("mouseup.create", function () {
                        ...
                        if (l != null) {
                            if (n == false) {
                                ...
                                var uiData = JSON.parse(sessionStorage.def_local_1);
                                var fromNode = {};
                                for (const key in uiData) {
                                    if (uiData.hasOwnProperty(key)) {
                                        const element = uiData[key];
                                        if (element.name == 'linker') {
                                            if (element.to.id == l.id) {
                                                fromNode = uiData[element.from.id];
                                            }
                                        }
                                    }
                                }
                                // parent.postMessage(JSON.stringify({ code: 0, value: l, from: fromNode }), '*'); //window.postMessage
                                ...
                                console.log("l =", l);
                                console.log("新建完成");
                                window.parent.postMessage(JSON.stringify({ code: 3, value: l }), "*");
                            }
                        } else if (!chartId) {
                ...
                if ((j.ctrlKey || j.metaKey) && j.keyCode == 83) {
                    ...
                    if ((j.ctrlKey || j.metaKey) && j.keyCode == 65) {//全选
                        ...
                        if (j.keyCode == 46 || j.keyCode == 8) {//删除
                            if (Designer.op.verifyRemoveShape())
    ...
    op: {
        ...
        linkerClickable: function (a) {
            ...
            b.unbind("click").bind("click", function (c) {
                ...
                //判断是否条件控制节点下面的连线
                var isProp_linker = false;
                var def_local_1_data = JSON.parse(sessionStorage.def_local_1);
                    ...
                    if (def_local_1_data.elements.hasOwnProperty(key)) {
                        ...
                        if (key === a.from.id) {
                            if (element.name == 'control') {
                                Designer.op.editShapeText(a, d);
                                isProp_linker = true;
                                parent.postMessage(JSON.stringify({ code: 0, value: a, type: 'control' }), '*'); //window.postMessage
                            }
                            if (element.name == 'branch') {
                                Designer.op.editShapeText(a, d);
                                isProp_linker = true;
                                parent.postMessage(JSON.stringify({ code: 0, value: a, type: 'branch' }), '*'); //window.postMessage
                            }
                        }
                    }
                }
                if (!isProp_linker) parent.postMessage(JSON.stringify({ code: -1, value: a }), '*');//如果不是控制节点的连线，那就隐藏右边所有的属性面板
            })
        ...
        shapeLinkable: function (c, a) {
            ...
            b.unbind("mousedown.link").bind("mousedown.link", function (h) {
                ...
                $(document).bind("mouseup.link", function () {
                    ...
                    console.log(JSON.parse(sessionStorage.def_local_1));
                    // 限制每个节点的入口只能有一个，在此时localStorage还没更新 start
                    if (to) {
                        var uiData = JSON.parse(sessionStorage.def_local_1);//本地的数据
                        for (const key in uiData.elements) {
                            if (uiData.elements.hasOwnProperty(key)) {
                                const element = uiData.elements[key];
                                if (element.name === 'linker' && element.to.id === to.id) {
                                    Utils.unselect();
                        ...
                        if (from.name !== 'start') {
                            setTimeout(function () {
                                parent.postMessage(JSON.stringify({ code: 4, value: { to: to, from: from } }), '*')//window.postMessage)
                            }, 500)
                ...
                if (sessionStorage.def_local_1) {
                    ...
                    switch (uiData.elements[fromId].name) {
                        ...
                        case 'rule'://如果是规则节点，限制只能拉一条线
                            ...
                            if (linkerCount >= 1) {
                                UI.showTip('规则节点只能有一条分支');
                                return null
                            }
                            break;
                        case 'scoreCard'://如果是评分卡节点，限制只能拉一条线
                            var linkerCount = 0;
                            for (const key in uiData.elements) {
                                if (uiData.elements.hasOwnProperty(key)) {
                                    const element = uiData.elements[key];
                            ...
                            if (linkerCount >= 1) {
                                UI.showTip('评分卡节点只能有一条分支');
                                return null
                            }
                            break;
                        case 'decisionTable'://如果是决策表节点，限制只能拉一条线
                            var linkerCount = 0;
                            for (const key in uiData.elements) {
                                if (uiData.elements.hasOwnProperty(key)) {
                                    const element = uiData.elements[key];
                            ...
                            if (linkerCount >= 1) {
                                UI.showTip('决策表节点只能有一条分支');
                                return null
                            }
                            break;
                        case 'ruleSet'://如果是规则集节点，限制只能拉一条线
                            var linkerCount = 0;
                            for (const key in uiData.elements) {
                                if (uiData.elements.hasOwnProperty(key)) {
                                    const element = uiData.elements[key];
                            ...
                            if (linkerCount >= 1) {
                                UI.showTip('规则集节点只能有一条分支');
                                return null
                            }
                            break;
                        case 'sql'://如果是脚本节点，限制只能拉一条线
                            var linkerCount = 0;
                            for (const key in uiData.elements) {
                                if (uiData.elements.hasOwnProperty(key)) {
                                    const element = uiData.elements[key];
            ...
            a.unbind("dblclick.edit_linker").bind("dblclick.edit_linker", function () {
                ...
                for (const key in def_local_1_data.elements) {
                    if (def_local_1_data.elements.hasOwnProperty(key)) {
                        ...
                        if (key === b.from.id) {
                            if (element.name == 'branch') {
                                if (b.to.id) {//限制该分支有to节点才进入判断
                                    parent.postMessage(JSON.stringify({ code: 1, value: b, type: 'branch' }), '*'); //window.postMessage
                                } else {
                                    UI.showTip('请先选择下游节点');
                                }
                            }
                        ...
                        if (linked) {
                            ...
                            if (d.to) {
                                ...
                                for (const key in uiData.elements) {
                                    if (uiData.elements.hasOwnProperty(key)) {
                                        const element = uiData.elements[key];
                                        if (element.name === 'linker' && element.to.id === d.to.id) {
                                            // Utils.unselect();
                                            // $('.shape_dashboard.menu').remove();
                                            // Model.add(d);
                                            // console.log("Model.add(g);")
                                            // Utils.selectShape(d.id)
                                            // // setTimeout(Designer.op.removeShape);
                                            // UI.showTip('节点入口最多只有一个');
                                            // return false
                                        }
                            ...
                            Model.update(d);
                            if (d.to.id) {
                                window.parent.postMessage(JSON.stringify({ code: 8, value: uiData.elements[d.to.id] }), "*");
                            }

                            //判断的连线的上游节点
                            var isControl_linker = false;
                            var isBranch_linker = false;
                            var def_local_1_data = JSON.parse(sessionStorage.def_local_1);
                            for (const key in def_local_1_data.elements) {
                                if (def_local_1_data.elements.hasOwnProperty(key)) {
                                    const element = def_local_1_data.elements[key];
                                    if (key === d.from.id) {
                                        switch (element.name) {
                                            case 'control':
                                                ...
                                                parent.postMessage(JSON.stringify({ code: 0, value: d }), '*'); //window.postMessage
                                                break;
                                            case 'branch':
                                                // Designer.op.editShapeText(a, d);
                                                isBranch_linker = true;
                                                parent.postMessage(JSON.stringify({ code: 4, value: { to: def_local_1_data.elements[d.to.id], from: def_local_1_data.elements[d.from.id] } }), '*'); //window.postMessage
                                                break;
                                            default:
                                                break;
                                        }
                            ...
                            if (!isControl_linker && !isBranch_linker) parent.postMessage(JSON.stringify({ code: -1, value: d }), '*');//如果不是控制节点和分支节点的连线，那就隐藏右边所有的属性面板
        ...
        verifyAddShape: function () {

        },
        verifyRemoveShape: function () {
            var selectedList = Utils.getSelected();
            console.log("删除选中的画布信息", selectedList);
            var uiData = JSON.parse(sessionStorage.def_local_1);//本地的数据
            console.log("删除前的uiData", uiData)
            var d = [];

            if (selectedList.length === 1) {//限制单个删除，并且从下到上删除
                if (uiData.elements[selectedList[0].id].name === 'linker') {
                    var linker_temp = uiData.elements[selectedList[0].id];
                    if (linker_temp.to.id && uiData.elements[linker_temp.to.id]) {
                        // UI.showTip('请从下往上单独删除');
                        // return false
                    }
                } else {
                    var node_temp = uiData.elements[selectedList[0].id];
                    if (node_temp.name === 'start') {
                        UI.showTip('开始节点不能删除');
                        return false
                    } else {
                        //遍历找linker
                        for (const key in uiData.elements) {
                            if (uiData.elements.hasOwnProperty(key)) {
                                const element = uiData.elements[key];
                                if (element.name === 'linker' && element.from.id && element.from.id === node_temp.id) {
                                    // UI.showTip('请从下往上单独删除');
                                    // return false
                                }
                            }
        ...
        removeShape: function () {
            ...
            if (selectedList.length > 0) {
                ...
                d = d.concat(f);
                sessionStorage.def_local_1_bk = sessionStorage.def_local_1;
                Model.remove(d)

                //与父窗口通讯，同步删除的数据（只传节点的id，连线不传）
                var ids = [];
                d.forEach(function (item, i) {
                    ids.push(item.id);
                })
                if (ids.length > 0)
                    parent.postMessage(JSON.stringify({ code: 2, value: ids }), "*")
            }
        },
        showTip: function (c) {
            ...
            if ($("#panel_" + a).length != 0) {
                ...
                function g(n) {
                    ...
                    for (const key in def_local_1_data.elements) {
                        if (def_local_1_data.elements.hasOwnProperty(key)) {
                            const element = def_local_1_data.elements[key];
                            if (element.name === 'linker' && element.to.id === x.id) {
                                linker = element;
                            }
                        }
                    }
                    if (linker) {
                        ////新建节点, 并且有连接线操作回调start
                        x_from = def_local_1_data.elements[linker.from.id];
                        window.parent.postMessage(JSON.stringify({ code: 5, value: { from: x_from, to: x } }), "*");
                        //新建节点, 并且有连接线操作回调end
                        for (const key in def_local_1_data.elements) {
                            if (def_local_1_data.elements.hasOwnProperty(key)) {
                                const element = def_local_1_data.elements[key];
                                if (key === linker.from.id) {
                                    if (element.name == 'control') {
                                        // Designer.op.editShapeText(a, d);
                                        isControl_linker = true;
                                        parent.postMessage(JSON.stringify({ code: 0, value: linker }), '*'); //window.postMessage
                                    }
                                }
                            }
                        }
    ...
    clipboard: {
        ...
        copy: function () {
            ...
            d.forEach(function (element) {
                if (element.name !== 'start') {// 去掉开始节点
                    filter_d.push(element);
                }
            })
            d = filter_d;
            ...
            for (var i = 0; i < d.length; i++) {
                ...
                switch (node.name) {
                    case 'output':
                        delete node.data.outPutNodeVO.id
                        delete node.data.outPutNodeVO.outPutId
                        break;
                    default:
                        removeDataAllId(node.data);
                        break;
                }
            }

            function removeDataAllId(obj) {//凡是在节点的data属性包含的id属性都去掉
                for (var key in obj) {
                    if (obj.hasOwnProperty(key)) {
                        ...
                        if (key === 'id') {
                            ...
                            if (Object.prototype.toString.call(element) == '[object Object]') {
                                removeDataAllId(element)
                            } else if (Object.prototype.toString.call(element) == '[object Array]') {
                                element.forEach(function (item) {
                                    removeDataAllId(item)
                                })
                            }
                        }
                    }
    ...
    painter: {
        ...
        renderText: function (a, u) {
            ...
            for (var n = 0; n < b.length; n++) {
                ...
                if (a.icon && l.siblings('.icon').length == 0) {
                    if (a.name === 'output') {
                        a.icon = '<img class="icon" src="/static/strategy/img/node/output-node.png" style="height: 18px;">'
                    }
                    l.after(a.icon);
                }
        ...
        renderLinkerText: function (h) {
            ...
            var def_local_1_data = JSON.parse(sessionStorage.def_local_1);
            if (h.from.id) {
                if (def_local_1_data.elements[h.from.id] && def_local_1_data.elements[h.from.id].name == 'branch') {
                    // textColor = "228, 75, 78";
                    if (h.status == '1') {
                        textColor = "34,202,47";
                    } else {
