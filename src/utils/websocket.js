 ...
 * timeout：连接超时时间
 * @type {module.webSocketConnection}
 */
module.exports = class webSocketConnection {
    ...
    createConnection = () => {
        const { ticket = '' } = this.param;
        let socketUrl = http.wsApiPrefix;
        if (process.env.NODE_ENV === 'production') {
            socketUrl = `ws://${ location.host }`;
        }
        socketUrl += `/websocket/${ ticket }`;
        this.socketUrl = socketUrl;
        let { timeout = 0 } = this.param;
    ...
    // 连接成功触发
    onopen = () => {
        console.log(`socket open success on ${this.socketUrl}`);
        let { socketOpen } = this.param;
        this.isSucces = true;  // true
